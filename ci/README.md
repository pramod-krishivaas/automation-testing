...
1. **GitHub Pages** — Settings → Pages → Build and deployment → Source:
   **Deploy from a branch**, branch **`gh-pages`**, folder **`/ (root)`**, Save.
   The report job pushes every report to that branch, so the push is the
   deployment. No custom domain is needed. After each push Pages takes about a
   minute to rebuild, so a brand-new report link can 404 briefly.
...
3. **Variables** (same page → *Variables*):
   ...
   | `REPORTS_BASE_URL` | `https://<owner>.github.io/<repo>` | Only if Pages uses a custom domain, so report links point at it. |
...
index is rebuilt, and the branch is pushed — which, with Pages deploying from
`gh-pages`, publishes it. Previous reports stay reachable at their own URLs.
